...
VAD.prototype._dequeueItem = function() { 
  var EVENT_MAP = ['error', 'silence', 'voice', 'noise']
    ...
    function evaluateAndDequeueNext(entry, err, res) {
	...
	var entry = this._processQueue.shift();
      
		
	binding.vad_processAudio(this._vad, entry.samples, entry.rate,
            evaluateAndDequeueNext.bind(this, entry))
    }
}
...
VAD.prototype.processAudio = function(samples, samplerate, callback) {
  ...
  if (!callback || typeof callback !== 'function') {
    ...
    function handleResult(callback, err, res) {
	var index = res + 1
    ...
    binding.vad_processAudio(this._vad, samples, samplerate,
	handleResult.bind(this, callback))
}
